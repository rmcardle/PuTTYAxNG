			 ...
			 struct Packet *pktin)
{
	...
	send_packet(ssh, SSH1_CMSG_USER, PKT_STR, s->username, PKT_END);
	{
	    ...
	    logevent(userlog);
	    if (flags_ & FLAG_INTERACTIVE &&
		(!((flags_ & FLAG_STDERR) && (flags_ & FLAG_VERBOSE)))) {
		strcat(userlog, "\r\n");
			     ...
			     struct Packet *pktin)
{
	...
	if (s->got_username && !ssh->cfg.change_username) {
	    ...
	    strncpy(s->username, ssh->cfg.username, sizeof(s->username));
	    s->username[sizeof(s->username)-1] = '\0';
	    if ((flags_ & FLAG_VERBOSE) || (flags_ & FLAG_INTERACTIVE)) {
	...
	while (1) {
		 ...
		 * output of (say) plink.)
		 */
		if (flags_ & (FLAG_VERBOSE | FLAG_INTERACTIVE)) {
		...
		if (s->response && s->responselen >= 5 &&
			...
			*s->q++ = SSH2_AGENTC_SIGN_REQUEST;
			...
			/* And finally the (zero) flags_ word. */
			PUT_32BIT(s->q, 0);
...
static const struct telnet_special *ssh_get_specials(void *handle)
{
    ...
    static struct telnet_special ssh_specials__FORLENOF[lenof(ssh2_transport_specials) +
					      lenof(ssh2_session_specials) +
					      lenof(specials_end)];
	#define ssh_specials		DEFINE_AX_GET_POINTER_ARRAY( ssh_specials, struct telnet_special *, sizeof( ssh_specials__FORLENOF ) )
...
#define ADD_SPECIALS(name) \
    do { \
	assert((i + lenof(name)) <= lenof(ssh_specials__FORLENOF)); \
