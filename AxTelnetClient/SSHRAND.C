...
static struct RandPool pool__FORSIZEOF;
#define pool			DEFINE_AX_GET_POINTER( pool, struct RandPool )

//int random_active = 0;
//long next_noise_collection;
#define random_active					DEFINE_AX_GET_POINTER( random_active, int )
#define next_noise_collection			DEFINE_AX_GET_POINTER( next_noise_collection, long )
...
static void random_stir(void)
{
	 ...
	 * anything.
	 */
	memcpy(block, pool.pool_, sizeof(block));
	...
	for (j = POOLSIZE; (j -= HASHSIZE) >= 0;) {
		...
		digest[k] ^= ((word32 *) (pool.pool_ + j))[k];
	    ...
	    for (k = 0; k < sizeof(digest) / sizeof(*digest); k++)
		((word32 *) (pool.pool_ + j))[k] = digest[k];
	}
    }
...
void random_add_noise(void *noise, int length)
{
	...
	for (i = 0; i < HASHSIZE; i++) {
	    pool.pool_[pool.poolpos++] ^= pool.incomingb[i];
...
{
    if (!random_active) {
	memset(&pool, 0, sizeof(pool__FORSIZEOF));    /* just to start with */
...
int random_byte(void)
{
    ...
    return pool.pool_[pool.poolpos++];
}
...
void random_get_savedata(void **data, int *len)
{
    ...
    random_stir();
    memcpy(buf, pool.pool_ + pool.poolpos, POOLSIZE / 2);
    *len = POOLSIZE / 2;
    *data = buf;
    random_stir();
}
