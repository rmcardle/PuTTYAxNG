...
#define timers				DEFINE_AX_GET_POINTER( timers, tree234 * )
#define timer_contexts		DEFINE_AX_GET_POINTER( timer_contexts, tree234 * )
#define now					DEFINE_AX_GET_POINTER( now, long )
...
long schedule_timer(int ticks, timer_fn_t fn, void *ctx)
{
	 ...
	 */
	timer_change_notify(first->now_);
    }
...
#error TIMING_SYNC definition incomplete
	...
	if (find234(timer_contexts, first->ctx, NULL) == NULL) {
	    ...
	    delpos234(timers, 0);
	    sfree(first);
	} else if (first->now_ - now <= 0) {
	    /*
	     * This timer is active and has reached its running
	     * time. Run it.
	     */
	    delpos234(timers, 0);
	    first->fn(first->ctx, first->now_);
	    sfree(first);
	} else {
	    /*
