			   ...
			   WPARAM wParam, LPARAM lParam)
{
	...
	static CHOOSECOLOR cc__FORSIZEOF;
	static DWORD custom__FORSIZEOF[16] = { 0 };    /* zero initialisers */

	#define cc				DEFINE_AX_GET_POINTER( cc, CHOOSECOLOR )
	#define custom			DEFINE_AX_GET_POINTER_ARRAY( custom, DWORD *, sizeof( custom__FORSIZEOF ) )

	cc.lStructSize = sizeof(cc__FORSIZEOF);
	cc.hwndOwner = dp->hwnd;
	cc.hInstance = (HWND) hinst_;
