...
#define ctrlbox				DEFINE_AX_GET_POINTER( ctrlbox, struct controlbox * )
...
#define ctrls_base					DEFINE_AX_GET_POINTER( ctrls_base, struct winctrls )
#define ctrls_panel					DEFINE_AX_GET_POINTER( ctrls_panel, struct winctrls )
#define dp							DEFINE_AX_GET_POINTER( dp, struct dlgparam )
#define events						DEFINE_AX_GET_POINTER( events, char ** )
#define nevents						DEFINE_AX_GET_POINTER( nevents, int )
#define negsize						DEFINE_AX_GET_POINTER( negsize, int )
...
void force_normal(HWND hwnd)
{
	...
	#define recurse					DEFINE_AX_GET_POINTER( recurse, int )
			      ...
			      WPARAM wParam, LPARAM lParam)
{
	...
	switch (LOWORD(wParam)) {
	  ...
	  case IDA_LICENCE:
	    EnableWindow(hwnd, 0);
	    DialogBox(hinst_, MAKEINTRESOURCE(IDD_LICENCEBOX),
		      hwnd, LicenceProc);
	    EnableWindow(hwnd, 1);
				       ...
				       WPARAM wParam, LPARAM lParam)
{
	...
	SendMessage(hwnd, WM_SETICON, (WPARAM) ICON_BIG,
		    (LPARAM) LoadIcon(hinst_, MAKEINTRESOURCE(IDI_CFGICON)));
	/*
	 * Centre the window.
	 */
	{			       /* centre the window */
	 ...
	 */
	{
	    ...
	    tvstatic = CreateWindowEx(0, "STATIC", "Cate&gory:",
				      ...
				      hwnd, (HMENU) IDCX_TVSTATIC, hinst_,
				      NULL);
	    ...
	    treeview = CreateWindowEx(WS_EX_CLIENTEDGE, WC_TREEVIEW, "",
				      ...
				      r.right - r.left, r.bottom - r.top,
				      hwnd, (HMENU) IDCX_TREEVIEW, hinst_,
				      NULL);
	    font = SendMessage(hwnd, WM_GETFONT, 0, 0);
...
{
    EnableWindow(hwnd, 0);
    DialogBox(hinst_, MAKEINTRESOURCE(IDD_ABOUTBOX), hwnd, AboutProc);
    EnableWindow(hwnd, 1);
    SetActiveWindow(hwnd);
}
...
void defuse_showwindow(void)
{
    ...
    {
	HWND hwnd;
	hwnd = CreateDialog(hinst_, MAKEINTRESOURCE(IDD_ABOUTBOX),
			    NULL, NullDlgProc);
...
int do_config(void)
{
    ...
    setup_config_box(ctrlbox, &sesslist_, FALSE, 0, 0);
    ...
    ret =
	SaneDialogBox(hinst_, MAKEINTRESOURCE(IDD_MAINBOX), NULL,
		  GenericMainDlgProc);
    get_sesslist(&sesslist_, FALSE);
...
int do_reconfig(HWND hwnd, int protcfginfo)
{
    ...
    ctrlbox = ctrl_new_box();
    setup_config_box(ctrlbox, &sesslist_, TRUE, cfg.protocol, protcfginfo);
    ...
    ret = SaneDialogBox(hinst_, MAKEINTRESOURCE(IDD_MAINBOX), NULL,
		  GenericMainDlgProc);
...
{
    if (!logbox) {
	logbox = CreateDialog(hinst_, MAKEINTRESOURCE(IDD_LOGBOX),
			      hwnd, LogProc);
