...
#ifdef _PUTTYAX_
#define PostQuitMessage			exit_Ax
#define exit					exit_Ax
void exit_Ax( int i )
{
...
#ifdef _PUTTYAX_
#define WndProc		WndProc_Putty
LRESULT CALLBACK WndProc_Ax(HWND, UINT, WPARAM, LPARAM);
#endif
...
#define extra_width			DEFINE_AX_GET_POINTER( extra_width, int )
#define extra_height		DEFINE_AX_GET_POINTER( extra_height, int )
#define font_width			DEFINE_AX_GET_POINTER( font_width, int )
#define font_height			DEFINE_AX_GET_POINTER( font_height, int )
#define font_dualwidth		DEFINE_AX_GET_POINTER( font_dualwidth, int )
#define offset_width		DEFINE_AX_GET_POINTER( offset_width, int )
#define offset_height		DEFINE_AX_GET_POINTER( offset_height, int )
#define was_zoomed			DEFINE_AX_GET_POINTER( was_zoomed, int )
#define prev_rows			DEFINE_AX_GET_POINTER( prev_rows, int )
#define prev_cols			DEFINE_AX_GET_POINTER( prev_cols, int )
...
#define caret_x			DEFINE_AX_GET_POINTER_INT( caret_x, int, -1 )
#define caret_y			DEFINE_AX_GET_POINTER_INT( caret_y, int, -1 )
...
static struct unicode_data ucsdata__FORSIZEOF;
...
static struct popup_menus__TYPE {
    HMENU menu;
    int specials_submenu_pos;
} popup_menus__FORLENOF[2];

#define kbd_codepage			DEFINE_AX_GET_POINTER( kbd_codepage, int )
#define ldisc					DEFINE_AX_GET_POINTER( ldisc, void * )
#define back					DEFINE_AX_GET_POINTER( back, Backend * )
#define backhandle				DEFINE_AX_GET_POINTER( backhandle, void * )
#define ucsdata_				DEFINE_AX_GET_POINTER( ucsdata_, struct unicode_data )
#define session_closed			DEFINE_AX_GET_POINTER( session_closed, int )
#define reconfiguring			DEFINE_AX_GET_POINTER( reconfiguring, int )
#define specials				DEFINE_AX_GET_POINTER( specials, struct telnet_special * )
#define n_specials				DEFINE_AX_GET_POINTER( n_specials, int )
#define timing_next_time		DEFINE_AX_GET_POINTER( timing_next_time, long )
#define popup_menus				DEFINE_AX_GET_POINTER_ARRAY( popup_menus, struct popup_menus__TYPE *, sizeof( popup_menus__FORLENOF ) )

enum { SYSMENU, CTXMENU };

//Config cfg;			       /* exported to windlg.c */
#define cfg			DEFINE_AX_GET_POINTER( cfg, Config )

//extern struct sesslist sesslist;       /* imported from windlg.c */
#define sesslist_				DEFINE_AX_GET_POINTER( sesslist_, struct sesslist )
...
static enum bold_mode__TYPE {
    BOLD_COLOURS, BOLD_SHADOW, BOLD_FONT
}; // bold_mode;
static enum und_mode__TYPE {
    UND_LINE, UND_FONT
}; // und_mode;
//static int descent;

#define fonts						DEFINE_AX_GET_POINTER_ARRAY( fonts, HFONT *, sizeof(HFONT) * FONT_MAXNO )
#define lfont						DEFINE_AX_GET_POINTER( lfont, LOGFONT )
#define fontflag					DEFINE_AX_GET_POINTER_ARRAY( fontflag, int *, sizeof(int) * FONT_MAXNO )
#define bold_mode					DEFINE_AX_GET_POINTER( bold_mode, enum bold_mode__TYPE )
#define und_mode					DEFINE_AX_GET_POINTER( und_mode, enum und_mode__TYPE )
#define descent						DEFINE_AX_GET_POINTER( descent, int )

#define NCFGCOLOURS 22
#define NEXTCOLOURS 240
#define NALLCOLOURS (NCFGCOLOURS + NEXTCOLOURS)

//static COLORREF colours_[NALLCOLOURS];
//static HPALETTE pal;
static LPLOGPALETTE logpal__FORSIZEOF;
...
#define colours_					DEFINE_AX_GET_POINTER_ARRAY( colours_, COLORREF *, sizeof(COLORREF) * NALLCOLOURS )
#define pal							DEFINE_AX_GET_POINTER( pal, HPALETTE )
#define logpal						DEFINE_AX_GET_POINTER( logpal, LPLOGPALETTE )
#define defpal						DEFINE_AX_GET_POINTER_ARRAY( defpal, RGBTRIPLE *, sizeof(RGBTRIPLE) * NALLCOLOURS )
#define caretbm						DEFINE_AX_GET_POINTER( caretbm, HBITMAP )
#define dbltime						DEFINE_AX_GET_POINTER( dbltime, int )
#define lasttime					DEFINE_AX_GET_POINTER( lasttime, int )
#define lastact						DEFINE_AX_GET_POINTER( lastact, int )
#define lastbtn						DEFINE_AX_GET_POINTER( lastbtn, Mouse_Button )
#define send_raw_mouse				DEFINE_AX_GET_POINTER( send_raw_mouse, int )
#define wheel_accumulator			DEFINE_AX_GET_POINTER( wheel_accumulator, int )
#define busy_status					DEFINE_AX_GET_POINTER_INT( busy_status, int, BUSY_NOT )
#define window_name					DEFINE_AX_GET_POINTER( window_name, char* )
#define icon_name					DEFINE_AX_GET_POINTER( icon_name, char* )
#define compose_state				DEFINE_AX_GET_POINTER( compose_state, int )
#define wm_mousewheel				DEFINE_AX_GET_POINTER_UINT( wm_mousewheel, UINT, WM_MOUSEWHEEL )
...
static void start_backend(void)
{
    ...
    error = back->init(NULL, &backhandle, &cfg,
		       ...
		       cfg.tcp_keepalives);
    back->provide_logctx(backhandle, logctx_);
    if (error) {
     ...
     */
    term_provide_resize_fn(term_, back->size, backhandle);

    /*
     * Set up a line discipline.
     */
    ldisc = ldisc_create(&cfg, term_, back, backhandle, NULL);
     ...
     */
    for (i = 0; i < lenof(popup_menus__FORLENOF); i++) {
...
#ifdef _PUTTYAX_
int WINAPI PuttyAx_WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show, HWND hwndAx, VOID* pvWndData)
#else
...
#endif
{
    ...
    hinst_ = inst;
    hwnd_ = NULL;
    flags_ = FLAG_VERBOSE | FLAG_INTERACTIVE;
	...
	/* Find the appropriate default port. */
	{
		...
		if (backends[i].protocol == default_protocol) {
		    default_port = backends[i].backend->default_port_;
		    break;
		}
	}
    ...
    memset(&ucsdata_, 0, sizeof(ucsdata__FORSIZEOF));
	...
	RECT		rectClient;
	GetClientRect( hwndAx, & rectClient );
	guess_width = rectClient.right;
	guess_height = rectClient.bottom;
    ...
    hwnd_ = CreateWindowEx(exwinmode, appname, appname,
			  winmode, x, y,
...
#ifdef _PUTTYAX_
	if ( hwnd_ )
	    SetWindowLong( hwnd_, GWL_USERDATA, (LONG) pvWndData );
#endif
    }
     ...
     * timer_change_notify() which will expect hwnd_ to exist.)
     */
    term_ = term_init(&cfg, &ucsdata_, NULL);
    logctx_ = log_init(NULL, &cfg);
    term_provide_logctx(term_, logctx_);
    term_size(term_, cfg.height, cfg.width, cfg.savelines);
    ...
    guess_width = extra_width + font_width * term_->cols;
    guess_height = extra_height + font_height * term_->rows;

    SetWindowPos(hwnd_, NULL, 0, 0, guess_width, guess_height,
		 SWP_NOMOVE | SWP_NOREDRAW | SWP_NOZORDER);

#else

	cfg.height = ( guess_height - extra_height ) / font_height;
	cfg.width = ( guess_width - extra_width ) / font_width;
    ...
    CreateCaret(hwnd_, caretbm, font_width, font_height);
	...
	popup_menus[SYSMENU].menu = GetSystemMenu(hwnd_, FALSE);
	popup_menus[CTXMENU].menu = CreatePopupMenu();
	AppendMenu(popup_menus[CTXMENU].menu, MF_ENABLED, IDM_PASTE, "&Paste");

	s = CreateMenu();
	get_sesslist(&sesslist_, TRUE);
	/* skip sesslist.sessions[0] == Default Settings */
	for (i = 1;
	     i < ((sesslist_.nsessions <= MENU_SAVED_MAX+1) ? sesslist_.nsessions
							   : MENU_SAVED_MAX+1);
	     i++)
	    AppendMenu(s, MF_ENABLED, IDM_SAVED_MIN + (i-1)*MENU_SAVED_STEP,
		       sesslist_.sessions[i]);

	for (j = 0; j < lenof(popup_menus__FORLENOF); j++) {
	...
	while (msg.message != WM_QUIT) {
	    ...
	    term_set_focus(term_, GetForegroundWindow() == hwnd_);
...
char *do_select(SOCKET skt, int startup)
{
    ...
    if (!hwnd_)
	return "do_select(): internal error (hwnd_==NULL)";
    if (p_WSAAsyncSelect(skt, hwnd_, msg, events) == SOCKET_ERROR) {
...
static void update_mouse_pointer(void)
{
	...
	#define forced_visible			DEFINE_AX_GET_POINTER( forced_visible, int )
	...
	SetClassLong(hwnd_, GCL_HCURSOR, (LONG)cursor);
...
static void enact_netevent(WPARAM wParam, LPARAM lParam)
{
	...
	#define reentering		DEFINE_AX_GET_POINTER( reentering, int )
	...
	if (cfg.close_on_exit == FORCE_ON ||
	    ...
	    close_session();
	    session_closed = TRUE;
	    MessageBox(hwnd_, "Connection closed by remote host",
...
static void init_palette(void)
{
	...
	if (cfg.try_palette && GetDeviceCaps(hdc, RASTERCAPS) & RC_PALETTE) {
	     ...
	     */
	    logpal = smalloc(sizeof(*logpal__FORSIZEOF)
			     - sizeof(logpal__FORSIZEOF->palPalEntry)
    ...
    if (pal)
	for (i = 0; i < NALLCOLOURS; i++)
	    colours_[i] = PALETTERGB(defpal[i].rgbtRed,
				    defpal[i].rgbtGreen,
				    defpal[i].rgbtBlue);
    else
	for (i = 0; i < NALLCOLOURS; i++)
	    colours_[i] = RGB(defpal[i].rgbtRed,
...
#define f(i,c,w,u) \
	...
	if (cset == OEM_CHARSET)
	    ucsdata_.font_codepage = GetOEMCP();
	else
	    if (TranslateCharsetInfo ((DWORD *) cset, &info, TCI_SRCCHARSET))
		ucsdata_.font_codepage = info.ciACP;
	else
	    ucsdata_.font_codepage = -1;

	GetCPInfo(ucsdata_.font_codepage, &cpinfo);
	ucsdata_.dbcs_screenfont = (cpinfo.MaxCharSize > 1);
    }
...
void request_resize(void *frontend, int w, int h)
{
    ...
    if (h == term_->rows && w == term_->cols) return;
	...
	//static RECT ss_2;
	#define first_time				DEFINE_AX_GET_POINTER_INT( first_time, int, 1 )
	#define ss_2					DEFINE_AX_GET_POINTER( ss_2, RECT )

	switch (first_time) {
	  case 1:
	    /* Get the size of the screen */
	    if (get_fullscreen_rect(&ss_2))
		/* first_time = 0 */ ;
	    else {
		first_time = 2;
		break;
	  ...
	  case 0:
	    ...
	    width = (ss_2.right - ss_2.left - extra_width) / 4;
	    height = (ss_2.bottom - ss_2.top - extra_height) / 6;
    ...
    term_size(term_, h, w, cfg.savelines);

    if (cfg.resize_action != RESIZE_FONT && !IsZoomed(hwnd_)) {
...
static void reset_window(int reinit) {
    ...
    if ( !reinit && 
	    (offset_width != (win_width-font_width*term_->cols)/2 ||
	     offset_height != (win_height-font_height*term_->rows)/2) ){
	offset_width = (win_width-font_width*term_->cols)/2;
	offset_height = (win_height-font_height*term_->rows)/2;
	InvalidateRect(hwnd_, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
	...
	if (cfg.resize_action != RESIZE_TERM) {
	    if (  font_width != win_width/term_->cols || 
		  font_height != win_height/term_->rows) {
		deinit_fonts();
		init_fonts(win_width/term_->cols, win_height/term_->rows);
		offset_width = (win_width-font_width*term_->cols)/2;
		offset_height = (win_height-font_height*term_->rows)/2;
		InvalidateRect(hwnd_, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
		 ...
		 */
		term_size(term_, win_height/font_height, win_width/font_width,
			  cfg.savelines);
		offset_width = (win_width-font_width*term_->cols)/2;
		offset_height = (win_height-font_height*term_->rows)/2;
		InvalidateRect(hwnd_, NULL, TRUE);
	...
	if (win_width != font_width*term_->cols + offset_width*2 ||
	    win_height != font_height*term_->rows + offset_height*2) {
	    ...
	    SetWindowPos(hwnd_, NULL, 0, 0, 
		         font_width*term_->cols + extra_width, 
			 font_height*term_->rows + extra_height,
			 SWP_NOMOVE | SWP_NOZORDER);
	}
	...
	if (win_width != font_width*term_->cols + offset_width*2 ||
	    ...
	    //static RECT ss_3;
		#define ss_3					DEFINE_AX_GET_POINTER( ss_3, RECT )

	    int width, height;
		
		get_fullscreen_rect(&ss_3);

	    width = (ss_3.right - ss_3.left - extra_width) / font_width;
	    height = (ss_3.bottom - ss_3.top - extra_height) / font_height;

	    /* Grrr too big */
	    if ( term_->rows > height || term_->cols > width ) {
		if (cfg.resize_action == RESIZE_EITHER) {
		    /* Make the font the biggest we can */
		    if (term_->cols > width)
			font_width = (ss_3.right - ss_3.left - extra_width)
			    / term_->cols;
		    if (term_->rows > height)
			font_height = (ss_3.bottom - ss_3.top - extra_height)
			    / term_->rows;
		    ...
		    if ( height > term_->rows ) height = term_->rows;
		    if ( width > term_->cols )  width = term_->cols;
		    term_size(term_, height, width, cfg.savelines);
#ifdef RDB_DEBUG_PATCH
		    debug((27, "reset_window() -> term_ resize to (%d,%d)",
			       height, width));
#endif
		}
	    }
    ...
    if (font_width != (win_width-cfg.window_border*2)/term_->cols || 
	font_height != (win_height-cfg.window_border*2)/term_->rows) {

	deinit_fonts();
	init_fonts((win_width-cfg.window_border*2)/term_->cols, 
		   (win_height-cfg.window_border*2)/term_->rows);
	offset_width = (win_width-font_width*term_->cols)/2;
	offset_height = (win_height-font_height*term_->rows)/2;
...
static void click(Mouse_Button b, int x, int y, int shift, int ctrl, int alt)
{
	...
	lastbtn = MBT_NOTHING;
	term_mouse(term_, b, translate_button(b), MA_CLICK,
		   x, y, shift, ctrl, alt);
	return;
    }
	...
	term_mouse(term_, b, translate_button(b), lastact,
		   x, y, shift, ctrl, alt);
    lasttime = thistime;
}
...
static void show_mouseptr(int show)
{
	...
	#define cursor_visible			DEFINE_AX_GET_POINTER_INT( cursor_visible, int, 1 )
...
//static int resizing;
#define resizing				DEFINE_AX_GET_POINTER( resizing, int )
...
void timer_change_notify(long next)
{
    ...
    KillTimer(hwnd_, TIMING_TIMER_ID);
    SetTimer(hwnd_, TIMING_TIMER_ID, ticks, NULL);
				...
				WPARAM wParam, LPARAM lParam)
{
	...
	#define ignore_clip				DEFINE_AX_GET_POINTER( ignore_clip, int )
	#define need_backend_resize		DEFINE_AX_GET_POINTER( need_backend_resize, int )
	#define fullscr_on_max			DEFINE_AX_GET_POINTER( fullscr_on_max, int )
	#define last_mousemove			DEFINE_AX_GET_POINTER( last_mousemove, UINT )
      ...
      case WM_CLOSE:
	{
		...
		1 ||
#endif		
		!cfg.warn_on_close || session_closed ||
		MessageBox(hwnd,
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
	...
	}
	{
		...
		term_mouse(term_, button, translate_button(button), MA_RELEASE,
      ...
      case WM_MOUSEMOVE:
	{
	    ...
	    //static WPARAM wp_2 = 0;
	    //static LPARAM lp_2 = 0;
		#define wp_2				DEFINE_AX_GET_POINTER( wp_2, WPARAM )
		#define lp_2				DEFINE_AX_GET_POINTER( lp_2, LPARAM )

	    if (wParam != wp_2 || lParam != lp_2 ||
		last_mousemove != WM_MOUSEMOVE) {
		show_mouseptr(1);
		wp_2 = wParam; lp_2 = lParam;
		last_mousemove = WM_MOUSEMOVE;
	    }
	}
	/*
	...
	if (wParam & (MK_LBUTTON | MK_MBUTTON | MK_RBUTTON) &&
		...
		b = MBT_RIGHT;
	    term_mouse(term_, b, translate_button(b), MA_DRAG,
		       TO_CHR_X(X_POS(lParam)),
		       TO_CHR_Y(Y_POS(lParam)), wParam & MK_SHIFT,
		       wParam & MK_CONTROL, is_alt_pressed());
	}
      ...
      case WM_NCMOUSEMOVE:
	{
	    ...
	    //static WPARAM wp_3 = 0;
	    //static LPARAM lp_3 = 0;
		#define wp_3			DEFINE_AX_GET_POINTER( wp_3, WPARAM )
		#define lp_3			DEFINE_AX_GET_POINTER( lp_3, LPARAM )

	    if (wParam != wp_3 || lParam != lp_3 ||
		last_mousemove != WM_NCMOUSEMOVE) {
		show_mouseptr(1);
		wp_3 = wParam; lp_3 = lParam;
		last_mousemove = WM_NCMOUSEMOVE;
	    }
	}
	noise_ultralight(lParam);
      ...
      case WM_PAINT:
	{
	     ...
	     */
	    term_paint(term_, hdc, 
		       (p.rcPaint.left-offset_width)/font_width,
		       (p.rcPaint.top-offset_height)/font_height,
		       (p.rcPaint.right-offset_width-1)/font_width,
		       (p.rcPaint.bottom-offset_height-1)/font_height,
		       !term_->window_update_pending);

	    if (p.fErase ||
	        p.rcPaint.left  < offset_width  ||
		p.rcPaint.top   < offset_height ||
		p.rcPaint.right >= offset_width + font_width*term_->cols ||
		p.rcPaint.bottom>= offset_height + font_height*term_->rows)
	    {
		HBRUSH fillcolour, oldbrush;
		HPEN   edge, oldpen;
		fillcolour = CreateSolidBrush (
				    colours_[ATTR_DEFBG>>ATTR_BGSHIFT]);
		...
		ExcludeClipRect(hdc, 
			...
			offset_width+font_width*term_->cols,
			offset_height+font_height*term_->rows);
	...
	if (cfg.resize_action != RESIZE_FONT && !is_alt_pressed()) {
	    ...
	    if ( !need_backend_resize && cfg.resize_action == RESIZE_EITHER &&
		    (cfg.height != term_->rows || cfg.width != term_->cols )) {
		/* 
	    ...
	    w = (width + term_->cols/2)/term_->cols;
	    h = (height + term_->rows/2)/term_->rows;
	    if ( r->right != r->left + w*term_->cols + ex_width)
		rv = 1;

	    if (wParam == WMSZ_LEFT ||
		wParam == WMSZ_BOTTOMLEFT || wParam == WMSZ_TOPLEFT)
		r->left = r->right - w*term_->cols - ex_width;
	    else
		r->right = r->left + w*term_->cols + ex_width;

	    if (r->bottom != r->top + h*term_->rows + ex_height)
		rv = 1;

	    if (wParam == WMSZ_TOP ||
		wParam == WMSZ_TOPRIGHT || wParam == WMSZ_TOPLEFT)
		r->top = r->bottom - h*term_->rows - ex_height;
	    else
		r->bottom = r->top + h*term_->rows + ex_height;
	...
	if (cfg.resize_action == RESIZE_DISABLED) {
		...
		if (wParam == SIZE_MAXIMIZED && !was_zoomed) {
		    was_zoomed = 1;
		    prev_rows = term_->rows;
		    prev_cols = term_->cols;
		    if (cfg.resize_action == RESIZE_TERM) {
			w = width / font_width;
			if (w < 1) w = 1;
		    ...
		    was_zoomed = 0;
		    if (cfg.resize_action == RESIZE_TERM)
			term_size(term_, prev_rows, prev_cols, cfg.savelines);
	...
	switch (LOWORD(wParam)) {
	  case SB_BOTTOM:
	    term_scroll(term_, -1, 0);
	    break;
	  case SB_TOP:
	    term_scroll(term_, +1, 0);
	    break;
	  case SB_LINEDOWN:
	    term_scroll(term_, 0, +1);
	    break;
	  case SB_LINEUP:
	    term_scroll(term_, 0, -1);
	    break;
	  case SB_PAGEDOWN:
	    term_scroll(term_, 0, +term_->rows / 2);
	    break;
	  case SB_PAGEUP:
	    term_scroll(term_, 0, -term_->rows / 2);
	    break;
	  ...
	  case SB_THUMBTRACK:
	    term_scroll(term_, 1, HIWORD(wParam));
	    break;
	}
	break;
	...
	if (message == wm_mousewheel || message == WM_MOUSEWHEEL) {
		...
		if (send_raw_mouse &&
		    ...
		    term_mouse(term_, b, translate_button(b),
			       MA_CLICK,
			       TO_CHR_X(X_POS(lParam)),
		      ...
		      unsigned long attr, int lattr)
{
	...
	#define IpDx			DEFINE_AX_GET_POINTER( IpDx, int* )
	#define IpDxLEN			DEFINE_AX_GET_POINTER( IpDxLEN, int )
    ...
    if (lattr != LATTR_NORM && x*2 >= term_->cols)
	return;
    ...
    if ((attr & TATTR_ACTCURS) && (cfg.cursor_type == 0 || term_->big_cursor)) {
	...
	if (lattr == LATTR_TOP || lattr == LATTR_BOT)
	    text_adjust *= 2;
	text[0] = ucsdata_.unitab_xterm['q'];
	if (attr & ATTR_UNDER) {
	    attr &= ~ATTR_UNDER;
    ...
    /* Only want the left half of double width lines */
    if (line_box.right > font_width*term_->cols+offset_width)
	line_box.right = font_width*term_->cols+offset_width;

    /* We're using a private area for direct to font. (512 chars.) */
    if (ucsdata_.dbcs_screenfont && (text[0] & CSET_MASK) == CSET_ACP) {
	/* Ho Hum, dbcs fonts are a PITA! */
	/* To display on W9x I have to convert to UCS */

	//static wchar_t *uni_buf = 0;
	//static int uni_len = 0;
	#define uni_buf			DEFINE_AX_GET_POINTER( uni_buf, wchar_t* )
	#define uni_len			DEFINE_AX_GET_POINTER( uni_len, int )
	...
	for(nlen = mptr = 0; mptr<len; mptr++) {
	    uni_buf[nlen] = 0xFFFD;
	    if (IsDBCSLeadByteEx(ucsdata_.font_codepage, (BYTE) text[mptr])) {
		char dbcstext[2];
		dbcstext[0] = text[mptr] & 0xFF;
		dbcstext[1] = text[mptr+1] & 0xFF;
		IpDx[nlen] += char_width;
	        MultiByteToWideChar(ucsdata_.font_codepage, MB_USEGLYPHCHARS,
	...
	#define directbuf				DEFINE_AX_GET_POINTER( directbuf, char* )
	#define directlen				DEFINE_AX_GET_POINTER( directlen, int )
	...
	//static int wlen = 0;
	#define wbuf			DEFINE_AX_GET_POINTER( wbuf, WCHAR* )
	#define wlen			DEFINE_AX_GET_POINTER( wlen, int )
	       ...
	       unsigned long attr, int lattr)
{
    ...
    if ((attr & TATTR_ACTCURS) && (ctype == 0 || term_->big_cursor)) {
	if (*text != UCSWIDE) {
    ...
    if ((attr & TATTR_PASCURS) && (ctype == 0 || term_->big_cursor)) {
	POINT pts[5];
	HPEN oldpen;
	...
	oldpen = SelectObject(hdc, CreatePen(PS_SOLID, 0, colours_[261]));
	Polyline(hdc, pts, 5);
	oldpen = SelectObject(hdc, oldpen);
	DeleteObject(oldpen);
	...
	if (attr & TATTR_ACTCURS) {
	    HPEN oldpen;
	    oldpen =
		SelectObject(hdc, CreatePen(PS_SOLID, 0, colours_[261]));
	    ...
	    for (i = 0; i < length; i++) {
		if (i % 2 == 0) {
		    SetPixel(hdc, startx, starty, colours_[261]);
		}
		startx += dx;
		starty += dy;
	    }
...
int char_width(Context ctx, int uc) {
    ...
    switch (uc & CSET_MASK) {
      case CSET_ASCII:
	uc = ucsdata_.unitab_line[uc & 0xFF];
	break;
      case CSET_LINEDRW:
	uc = ucsdata_.unitab_xterm[uc & 0xFF];
	break;
      case CSET_SCOACS:
	uc = ucsdata_.unitab_scoacs[uc & 0xFF];
	break;
    }
    if (DIRECT_FONT(uc)) {
	if (ucsdata_.dbcs_screenfont) return 1;
			...
			unsigned char *output)
{
	...
	#define alt_sum				DEFINE_AX_GET_POINTER( alt_sum, int )

    HKL kbd_layout = GetKeyboardLayout(0);

    /* keys is for ToAsciiEx. There's some ick here, see below. */

    static WORD keys__FORSIZEOF[3];
    //static int compose_char = 0;
    //static WPARAM compose_key_2 = 0;
	#define keys				DEFINE_AX_GET_POINTER_ARRAY( keys, WORD*, sizeof( keys__FORSIZEOF ) )
	#define compose_char		DEFINE_AX_GET_POINTER( compose_char, int )
	#define compose_key_2		DEFINE_AX_GET_POINTER( compose_key_2, WPARAM )
...
#define SHOW_TOASCII_RESULT
	{			       /* Tell us all about key events */
	    ...
	    //static BYTE oldstate__FORSIZEOF[256];
	    //static int first = 1;
	    //static int scan;
		//#define oldstate			DEFINE_AX_GET_POINTER_ARRAY( oldstate, BYTE*, sizeof( oldstate__FORSIZEOF ) )
		//#define first				DEFINE_AX_GET_POINTER_INT( first, int, 1 )
		//#define scan				DEFINE_AX_GET_POINTER( scan, int )
	...
	if ((cfg.funky_type == FUNKY_VT400 ||
	     (cfg.funky_type <= FUNKY_LINUX && term_->app_keypad_keys &&
	      !cfg.no_applic_k))
	...
	if (cfg.compose_key) {
	    if (wParam == VK_MENU && (HIWORD(lParam) & KF_EXTENDED))
		compose_key_2 = wParam;
	}
	if (wParam == VK_APPS)
	    compose_key_2 = wParam;
    }

    if (wParam == compose_key_2) {
	if (compose_state == 0
    ...
    if (left_alt || (term_->app_keypad_keys && !cfg.no_applic_k
	...
	if (wParam == VK_PRIOR && shift_state == 1) {
	    SendMessage(hwnd_, WM_VSCROLL, SB_PAGEUP, 0);
	    return 0;
	}
	if (wParam == VK_PRIOR && shift_state == 2) {
	    SendMessage(hwnd_, WM_VSCROLL, SB_LINEUP, 0);
	    return 0;
	}
	if (wParam == VK_NEXT && shift_state == 1) {
	    SendMessage(hwnd_, WM_VSCROLL, SB_PAGEDOWN, 0);
	    return 0;
	}
	if (wParam == VK_NEXT && shift_state == 2) {
	    SendMessage(hwnd_, WM_VSCROLL, SB_LINEDOWN, 0);
	    return 0;
	}
	...
	if (left_alt && wParam == VK_SPACE && cfg.alt_space) {
	    SendMessage(hwnd_, WM_SYSCOMMAND, SC_KEYMENU, 0);
	    return -1;
	}
	...
	if (!left_alt) {
		...
		(cfg.funky_type <= FUNKY_LINUX &&
		 term_->app_keypad_keys && !cfg.no_applic_k)) switch (wParam) {
		  case VK_EXECUTE:
		    xkey = 'P';
		    break;
		  case VK_DIVIDE:
	    ...
	    if (term_->app_keypad_keys && !cfg.no_applic_k)
		switch (wParam) {
		  case VK_NUMPAD0:
		    xkey = 'p';
		    break;
	...
	if (shift_state == 0 && wParam == VK_RETURN && term_->cr_lf_return) {
	    *p++ = '\r';
	    *p++ = '\n';
	    return p - output;
	}
	...
	if (term_->vt52_mode && code > 0 && code <= 6) {
	...
	if ((term_->vt52_mode || cfg.funky_type == FUNKY_VT100P) && code >= 11 && code <= 24) {
	    int offt = 0;
	    if (code > 15)
		offt++;
	    if (code > 21)
	 ...
	 */
	{
		...
		if (term_->vt52_mode)
		    ...
		    int app_flg = (term_->app_cursor_keys && !cfg.no_applic_c);
#if 0
		    /*
	...
	if (r == 1 && !key_down) {
	    if (alt_sum) {
		if (in_utf(term_) || ucsdata_.dbcs_screenfont)
		    debug((", (U+%04x)", alt_sum));
		else
		    debug((", LCH(%d)", alt_sum));
	    } else {
	...
	else if (keys[0] && (in_utf(term_) || ucsdata_.dbcs_screenfont))
	    keys[0] = 10;
    }
...
void set_title(void *frontend, char *title)
{
    ...
    if (cfg.win_name_always || !IsIconic(hwnd_))
...
void write_aclip(void *frontend, char *data, int len, int must_deselect)
{
    ...
    if (!must_deselect)
	SendMessage(hwnd_, WM_IGNORE_CLIP, TRUE, 0);
    ...
    if (!must_deselect)
	SendMessage(hwnd_, WM_IGNORE_CLIP, FALSE, 0);
}
...
void get_clip(void *frontend, wchar_t ** p, int *len)
{
	...
	#define clipdata				DEFINE_AX_GET_POINTER( clipdata, HGLOBAL )
	#define converted				DEFINE_AX_GET_POINTER( converted, wchar_t* )
...
void optimised_move(void *frontend, int to, int from, int lines)
{
    ...
    r.left = offset_width;
    r.right = offset_width + term_->cols * font_width;
    r.top = offset_height + min * font_height;
    r.bottom = offset_height + (max + lines) * font_height;
    ScrollWindow(hwnd_, 0, (to - from) * font_height, &r, &r);
}
#endif
...
#define next_flash				DEFINE_AX_GET_POINTER( next_flash, long )
#define flashing				DEFINE_AX_GET_POINTER( flashing, int )
...
static void flash_window(int mode)
{
	...
	if (!flashing) {
	    ...
	    next_flash = schedule_timer(450, flash_window_timer, hwnd_);
	}

    } else if ((mode == 1) && (cfg.beep_ind == B_IND_FLASH)) {
	/* maintain */
	if (flashing) {
	    FlashWindow(hwnd_, TRUE);	/* toggle */
	    next_flash = schedule_timer(450, flash_window_timer, hwnd_);
	}
    }
}
...
void beep(void *frontend, int mode)
{
	...
	#define lastbeep				DEFINE_AX_GET_POINTER( lastbeep, long )
	...
	if (!PlaySound(cfg.bell_wavefile.path, NULL,
	    ...
	    MessageBox(hwnd_, buf, otherbuf,
		       MB_OK | MB_ICONEXCLAMATION);
	    cfg.beep = BELL_DEFAULT;
	}
    } else if (mode == BELL_PCSPEAKER) {

	//static long lastbeep_2 = 0;
	#define lastbeep_2				DEFINE_AX_GET_POINTER( lastbeep_2, long )

	long beepdiff;

	beepdiff = GetTickCount() - lastbeep_2;
	if (beepdiff >= 0 && beepdiff < 50)
	    return;
...
void set_zorder(void *frontend, int top)
{
    ...
    if (cfg.alwaysontop)
	return;			       /* ignore */
    SetWindowPos(hwnd_, top ? HWND_TOP : HWND_BOTTOM, 0, 0, 0, 0,
		 SWP_NOMOVE | SWP_NOSIZE);
}
...
static int is_full_screen()
{
    if (!IsZoomed(hwnd_))
	return FALSE;
    if (GetWindowLong(hwnd_, GWL_STYLE) & WS_CAPTION)
	return FALSE;
    return TRUE;
}
...
static void make_full_screen()
{
    ...
    SetWindowPos(hwnd_, HWND_TOP, ss.left, ss.top,
			ss.right - ss.left,
			ss.bottom - ss.top,
			SWP_FRAMECHANGED);

    /* Tick the menu item in the System menu. */
    CheckMenuItem(GetSystemMenu(hwnd_, FALSE), IDM_FULLSCREEN,
		  MF_CHECKED);
}
...
static void clear_full_screen()
{
    ...
    style = oldstyle = GetWindowLong(hwnd_, GWL_STYLE);
...
static void flip_full_screen()
{
    if (is_full_screen()) {
	ShowWindow(hwnd_, SW_RESTORE);
    } else if (IsZoomed(hwnd_)) {
	make_full_screen();
    } else {
	SendMessage(hwnd_, WM_FULLSCR_ON_MAX, 0, 0);
	ShowWindow(hwnd_, SW_MAXIMIZE);
    }
}
...
{
    return term_data(term_, is_stderr, data, len);
}
			     ...
			     void *callback_ctx, void *data, int len)
{
    ...
    c->data = data;
    c->len = len;
    PostMessage(hwnd_, WM_AGENT_CALLBACK, 0, (LPARAM)c);
}
