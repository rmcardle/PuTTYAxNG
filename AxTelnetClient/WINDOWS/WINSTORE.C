...
static char seedpath__FORSIZEOF[2 * MAX_PATH + 10] = "\0";
#define seedpath				DEFINE_AX_GET_POINTER_ARRAY( seedpath, char *, sizeof( seedpath__FORSIZEOF ) )
...
static void get_seedpath(void)
{
    ...
    size = sizeof(seedpath__FORSIZEOF);
	...
	len =
	    GetEnvironmentVariable("HOMEDRIVE", seedpath,
				   sizeof(seedpath__FORSIZEOF));
	ret =
	    GetEnvironmentVariable("HOMEPATH", seedpath + len,
				   sizeof(seedpath__FORSIZEOF) - len);
	if (ret == 0) {		       /* probably win95; store in \WINDOWS */
	    GetWindowsDirectory(seedpath, sizeof(seedpath__FORSIZEOF));
	    len = strlen(seedpath);
	} else
	    len += ret;
